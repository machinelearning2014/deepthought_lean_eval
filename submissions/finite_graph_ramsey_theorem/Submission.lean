          ...
          · rw [SimpleGraph.IsClique]
            ...
            have : G.IsNClique 2 {x, y} := by
              ...
              · rw [SimpleGraph.IsClique]
                ...
                · exact G.symm hadj
              · simp [hxy]
            have not_cf : ¬ G.CliqueFree 2 := Helpers.not_cliqueFree_of_exists_isNClique G 2 {x, y} this
            exact not_cf hG
          · simp
        exact Helpers.not_cliqueFree_of_exists_isNClique Gᶜ s' Finset.univ this
      · left; exact hG
    · by_cases h2s : s' = 2
          ...
          have : G.IsNClique r' (Finset.univ : Finset (Fin r')) := by
            ...
            · rw [SimpleGraph.IsClique]
              ...
              have : Gᶜ.IsNClique 2 {x, y} := by
                ...
                · simp [hxy]
              have not_cf : ¬ Gᶜ.CliqueFree 2 := Helpers.not_cliqueFree_of_exists_isNClique Gᶜ 2 {x, y} this
              exact not_cf hG
            · simp
          exact Helpers.not_cliqueFree_of_exists_isNClique G r' Finset.univ this
        · right; exact hG
      · have h3r : 3 ≤ r' := by omega
        ...
        have hn1pos : n1 ≠ 0 := by
          ...
          have h_cf_r : (⊤ : SimpleGraph (Fin 0)).CliqueFree (r'-1) := by
            have : 1 ≤ r'-1 := by omega
            exact Helpers.cliqueFree_on_fin0 ⊤ (r'-1) this
          have h_cf_s : (⊤ : SimpleGraph (Fin 0))ᶜ.CliqueFree s' := by
            have : 1 ≤ s' := by omega
            exact Helpers.cliqueFree_on_fin0 (⊤ : SimpleGraph (Fin 0))ᶜ s' this
        ...
        have hn2pos : n2 ≠ 0 := by
          ...
          have h_cf_r : (⊤ : SimpleGraph (Fin 0)).CliqueFree r' := by
            have : 1 ≤ r' := by omega
            exact Helpers.cliqueFree_on_fin0 ⊤ r' this
          have h_cf_s : (⊤ : SimpleGraph (Fin 0))ᶜ.CliqueFree (s'-1) := by
            have : 1 ≤ s'-1 := by omega
            exact Helpers.cliqueFree_on_fin0 (⊤ : SimpleGraph (Fin 0))ᶜ (s'-1) this
        ...
        classical
          ...
          · have hA_card_ge' : n1 ≤ A.card := hA_card_ge
            ...
            let f_emb : Fin n1 ↪ Fin n := Helpers.subsetEmbedding n n1 A_sub hA_card
            have f_emb_inj : Function.Injective f_emb := f_emb.injective
            have f_emb_mem (a : Fin n1) : f_emb a ∈ A_sub := by
              simpa [f_emb] using Helpers.subsetEmbedding_mem n n1 A_sub hA_card a
            have h1_case : ¬ (Helpers.pullback G f_emb).CliqueFree (r'-1) ∨ ¬ ((Helpers.pullback G f_emb)ᶜ).CliqueFree s' :=
              h1 (Helpers.pullback G f_emb)
            rcases h1_case with (hclique | hcompl)
            · rcases (Helpers.not_cliqueFree_iff_exists_isNClique (Helpers.pullback G f_emb) (r'-1)).mp hclique with ⟨t, ht⟩
              have hG_clique : G.IsNClique (r'-1) (Finset.image f_emb t) :=
                Helpers.image_pullback_isNClique G f_emb f_emb_inj (r'-1) t ht
              ...
              have hG_clique_temp : G.IsNClique ((r'-1)+1) (insert v (Finset.image f_emb t)) :=
                Helpers.isNClique_insert_adjacent' G (r'-1) (Finset.image f_emb t) v hG_clique hv_not_img h_adj_all
              ...
              left; exact Helpers.not_cliqueFree_of_exists_isNClique G r' (insert v (Finset.image f_emb t)) hG_clique_r'
            · rcases (Helpers.not_cliqueFree_iff_exists_isNClique ((Helpers.pullback G f_emb)ᶜ) s').mp hcompl with ⟨t, ht⟩
              have hG_compl_clique : Gᶜ.IsNClique s' (Finset.image f_emb t) :=
                Helpers.image_pullback_compl_isNClique G f_emb f_emb_inj s' t ht
              right; exact Helpers.not_cliqueFree_of_exists_isNClique Gᶜ s' (Finset.image f_emb t) hG_compl_clique
          · have hA_card_lt : A.card < n1 := by omega
            ...
            let g_emb : Fin n2 ↪ Fin n := Helpers.subsetEmbedding n n2 B_sub hB_card
            have g_emb_inj : Function.Injective g_emb := g_emb.injective
            have g_emb_mem (a : Fin n2) : g_emb a ∈ B_sub := by
              simpa [g_emb] using Helpers.subsetEmbedding_mem n n2 B_sub hB_card a
            have h2_case : ¬ (Helpers.pullback G g_emb).CliqueFree r' ∨ ¬ ((Helpers.pullback G g_emb)ᶜ).CliqueFree (s'-1) :=
              h2 (Helpers.pullback G g_emb)
            rcases h2_case with (hclique | hcompl)
            · rcases (Helpers.not_cliqueFree_iff_exists_isNClique (Helpers.pullback G g_emb) r').mp hclique with ⟨t, ht⟩
              have hG_clique : G.IsNClique r' (Finset.image g_emb t) :=
                Helpers.image_pullback_isNClique G g_emb g_emb_inj r' t ht
              left; exact Helpers.not_cliqueFree_of_exists_isNClique G r' (Finset.image g_emb t) hG_clique
            · rcases (Helpers.not_cliqueFree_iff_exists_isNClique ((Helpers.pullback G g_emb)ᶜ) (s'-1)).mp hcompl with ⟨t, ht⟩
              have hG_compl_clique : Gᶜ.IsNClique (s'-1) (Finset.image g_emb t) :=
                Helpers.image_pullback_compl_isNClique G g_emb g_emb_inj (s'-1) t ht
              ...
              have hG_compl_clique_temp : Gᶜ.IsNClique ((s'-1)+1) (insert v (Finset.image g_emb t)) :=
                Helpers.isNClique_insert_adjacent' Gᶜ (s'-1) (Finset.image g_emb t) v hG_compl_clique hv_not_img h_adj_all_compl
              ...
              right; exact Helpers.not_cliqueFree_of_exists_isNClique Gᶜ s' (insert v (Finset.image g_emb t)) hG_compl_clique_s'
